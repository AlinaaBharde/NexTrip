...
function UserRestaurants({ planid }) {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const response = await axios.get('https://nextrip-api.onrender.com/api/restaurants/display/' + planid);
  ...
  const handleDelete = async (restaurantId) => {
    try {
      await axios.delete(`https://nextrip-api.onrender.com/api/restaurants/delete/${planid}`, {
        headers: {
  ...
  return (
    <div className="grid grid-cols-1  gap-2 mt-6 mb-12 ml-4 bg-[#f5f5f5]">
      <Button className=" w-20 mx-auto mb-2 bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-full shadow z-10 font-bold"><Link to={`https://neural-nexus-frontend.vercel.app/plan/${planid}`} className='font-bold text-white'
      >
        Update
      </Link>
      </Button>
