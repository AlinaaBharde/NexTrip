...
export default function Flights({ locationName, startDate, endDate, adults, index }) {
  ...
  function renderFilter() {
    return (
      <div className='relative sm:bg-cover bg-center sm:h-80 w-screen' style={{ backgroundImage: `url(${srcimg})` }}>
        <div className='absolute inset-0 bg-black bg-opacity-20 flex flex-col items-center justify-center'>
          <div className="text-center text-white sm:text-[40px] p-1 font-[700]">The best flight offers from anywhere, to everywhere</div>
          <Card className='bg-opacity-15 backdrop-blur-sm w-full mx-auto'>
  ...
  return (
    <div className='w-screen flex flex-col p-0 mt-[50] sm:m-auto  justify-center align-middle items-center'>
      ...
      {flights && flights.length === 0 ? (
        <p className=" ml-10 container border rounded-md shadow bg-transparent text-indigo-700 p-6 pl-12  mt-6 mb-12 font-bold text-7xl">Sorry, No Flights Available.
        </p>
      ) : (
        <div className='flex flex-col items-center px-10 py-5 w-full m-0'>
          {flights && flights.map((flight, index) => (
            <Card key={index} className="bg-white flex flex-col rounded shadow-lg flex-grow w-full mx-auto m-2">
              ...
              <div className="m-0 p-0 flex sm:flex-row mx-1 sm:justify-between flex-wrap w-full items-center justify-center align-middle">
                <div className='flex flex-col place-items-center p-0'>
